...
from utils import get_top_indices, load_ridge_models, model_base_name
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Score Unsplash 25k image patches with Qwen3-VL and save PT/CSV outputs.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--model-path", default=DEFAULT_MODEL_PATH)
    parser.add_argument("--image-dir", default=DEFAULT_IMAGE_DIR)
    parser.add_argument("--output-dir", default=None)
    parser.add_argument("--activations-name", default="unsplash25k_patch_scores.pt")
    parser.add_argument("--stats-name", default="unsplash25k_patch_stats.csv")
...
def encode_sample(processor, image_path: str, prompt: str) -> Dict:
    with Image.open(image_path) as img:
        width, height = img.size
    grid_h, grid_w = vision_grid_hw(height, width, processor)
...
def main() -> None:
    ...
    for image_batch in progress:
        samples = [encode_sample(processor, image_path, args.prompt) for image_path in image_batch]
        ...
        progress.set_postfix(processed=len(all_image_paths))
