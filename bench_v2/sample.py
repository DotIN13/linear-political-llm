...
Ported into ``bench_v2`` from ``bench/sample.py`` (stdlib only) and extended with
``--bucket-by``:

* ``mean``    -- fixed ``+/-0.5`` thresholds on the per-image patch mean.
* ``extreme`` -- equal-count terciles on the per-image *signed extreme patch
  score*, i.e. whichever of the image's min/max patch scores sits further from
  zero (``[-10, 5, 5, 3]`` -> ``-10``). That metric runs about -4..+4 with
  median ~+1, so the fixed ``+/-0.5`` cut would leave the middle bucket empty.
"""
    ...
    with open(path, encoding="utf-8", newline="") as handle:
        for raw in csv.DictReader(handle):
            rows.append({
                ...
                "image_extreme": signed_extreme(float(raw["image_min"]), float(raw["image_max"])),
...
# What each bucket is a bucket *of*.
#   mean    -- the fixed +/-0.5 thresholds on the per-image patch mean (round 4).
#   extreme -- equal-count terciles on the per-image signed extreme patch score,
#              because that metric runs about -4..+4 with median ~+1, so the
#              fixed +/-0.5 cut leaves an empty middle bucket.
BUCKET_FIELD = {"mean": "image_mean", "extreme": "image_extreme"}
...
def tercile_cuts(values: Sequence[float]) -> Tuple[float, float]:
    """(p33, p66) for equal-count low/mid/high buckets."""
    ordered = sorted(values)
    n = len(ordered)
    if n < 3:
        raise ValueError("need at least 3 values for terciles")
    return ordered[n // 3], ordered[(2 * n) // 3]


def assign_buckets(rows: Sequence[Dict[str, Any]],
                   bucket_by: str = "mean") -> List[Dict[str, Any]]:
    """Tag every row with its bucket name and the ordinal (-1/0/+1) that carries
    the row's `stratum` slot. The bucketing field itself is untouched: the bucket
    only guarantees the two tails are covered; the main analysis stays continuous.

    ``bucket_by="mean"`` keeps the round-4 fixed thresholds; ``"extreme"`` cuts
    the signed extreme patch score into equal-count terciles.
    """
    if bucket_by not in BUCKET_FIELD:
        raise ValueError(f"unknown bucket_by {bucket_by!r}; expected {tuple(BUCKET_FIELD)}")
    field = BUCKET_FIELD[bucket_by]
    if bucket_by == "extreme":
        lo, hi = tercile_cuts([row[field] for row in rows])
    ...
    for row in rows:
        row = dict(row)
        bucket = label(row[field])
        row["bucket"] = bucket
        row["bucket_by"] = bucket_by
...
def make_bucket_items(
    ...
    ``bucket_by`` selects the bucketing field: ``"mean"`` (fixed +/-0.5 on the
    per-image patch mean) or ``"extreme"`` (equal-count terciles on the per-image
    signed extreme patch score).
    """
    rng = random.Random(seed)
    rows = assign_buckets(rows, bucket_by)
    field = BUCKET_FIELD[bucket_by]
    ...
    for bucket in BUCKETS:
        ...
        for group_index, group in enumerate(groups):
            split = splits[group_index % len(splits)]
            scores = [row["image_mean"] for row in group]
            extremes = [row["image_extreme"] for row in group]
            cats = sorted({c for row in group for c in row["categories"]})
            n_objects = [row["n_objects"] for row in group]
            ...
            item = {
                ...
                "covariates": {
                    ...
                    "categories": cats,
                    "image_mean_mean": sum(scores) / len(scores),
                    "image_extreme": extremes,
                    "image_extreme_mean": sum(extremes) / len(extremes),
                },
            }
            out[split].append(item)
            counters[bucket] += 1
        ...
        if chosen:
            ...
            profile_rows.append({
                ...
                "image_mean_max": max(r["image_mean"] for r in chosen),
                "image_mean_mean": sum(r["image_mean"] for r in chosen) / len(chosen),
                "image_extreme_min": min(r["image_extreme"] for r in chosen),
                "image_extreme_max": max(r["image_extreme"] for r in chosen),
                "image_extreme_mean": sum(r["image_extreme"] for r in chosen) / len(chosen),
                "n_objects_median": _median([r["n_objects"] for r in chosen]),
...
def write_bucket_profile(profile: Dict[str, Any], path: str) -> str:
    ...
    with open(path, "w", encoding="utf-8", newline="") as handle:
        ...
        writer.writerow(["bucket", "n_images_available", "n_images_selected",
                         ...
                         "image_extreme_min", "image_extreme_mean", "image_extreme_max",
        ...
        for row in profile["buckets"]:
            writer.writerow([
                ...
                f"{row['image_mean_min']:.4f}", f"{row['image_mean_mean']:.4f}",
                f"{row['image_mean_max']:.4f}",
                f"{row['image_extreme_min']:.4f}", f"{row['image_extreme_mean']:.4f}",
                f"{row['image_extreme_max']:.4f}", row["n_objects_median"],
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    from bench_v2.store import git_rev, write_items

    parser = argparse.ArgumentParser(
        description="build frozen stimulus items from the LVIS score CSV")
    parser.add_argument("--pool", default="lvis", choices=["lvis"])
    parser.add_argument("--stats", default=DEFAULT_STATS_CSV)
    parser.add_argument("--lvis-json", default=DEFAULT_LVIS_JSON)
    parser.add_argument("--lvis-cache", default=DEFAULT_LVIS_CACHE)
    parser.add_argument("--bucket-by", default="mean", choices=sorted(BUCKET_FIELD),
                        help="mean: fixed +/-0.5 on image_mean; "
                             "extreme: equal-count terciles on the signed extreme patch score")
    parser.add_argument("--per-bucket", type=int, default=400)
    parser.add_argument("--images-per-item", type=int, default=3)
    parser.add_argument("--filters", default=DEFAULT_FILTERS)
    parser.add_argument("--split", default="explore,confirm")
    ...
    cache_path = os.path.abspath(args.lvis_cache)
    if not os.path.exists(cache_path):
        build_lvis_meta_cache(os.path.abspath(args.lvis_json), cache_path)

    print(f"[sample] stats: {stats_path}")
    rows = load_stats(stats_path, limit=args.limit)
    print(f"[sample] loaded {len(rows)} scored images")
    meta = load_lvis_meta(cache_path)
    print(f"[sample] loaded LVIS meta for {len(meta)} images")

    filters = parse_filters(args.filters)
    kept, dropped = apply_filters(rows, meta, filters)
    ...
    items, profile = make_bucket_items(
        kept, per_bucket=args.per_bucket, images_per_item=args.images_per_item,
        splits=splits, seed=args.seed, bucket_by=args.bucket_by,
    )
    ...
    profile_path = os.path.join(out_dir, f"bucket_profile{suffix}.csv")
    write_bucket_profile(profile, profile_path)
    ...
    with open(manifest_path, "w", encoding="utf-8") as handle:
        json.dump({"command": sys.argv, "code_rev": git_rev(ROOT_DIR),
                   ...
                   "n_scored_images": len(rows), "dropped": dict(dropped),
                   "pool_after_filters": len(kept), "profile": profile},
                  handle, indent=2, sort_keys=True, default=str)
    print(f"[sample] wrote {profile_path}")
    print(f"\nbucket profile ({profile['bucket_by']} bucketing):")
    for row in profile["buckets"]:
        cats = ", ".join(f"{n}({c})" for n, c in row["top_categories"][:5])
        print(f"  {row['bucket']:>4}  n={row['n_images_selected']:>5}  "
              f"extreme={row['image_extreme_mean']:+.3f} "
              f"[{row['image_extreme_min']:+.3f},{row['image_extreme_max']:+.3f}]  "
