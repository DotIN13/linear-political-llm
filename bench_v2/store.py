...
from bench_v2.types import Conversation, canonical_variant
...
#
# ``bench_v2/tasks/**/*.py`` is deliberately NOT here. A pilot is the experiment
# driver, not the instrument: editing build()/read()/summary() must not invalidate
# a trial, while editing a prompt file or a helper must. So what is hashed is the
# adaptor code, every helper, the value types and store, and the prompt material
# themselves -- never the pilot that arranges them.
#
# Everything else in the repo -- README, docs, notebooks, the old bench/ tree --
# can change freely without invalidating a single bench_v2 trial.
MEASUREMENT_GLOBS = (
