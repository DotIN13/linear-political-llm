...
- **Judge default: `gpt-5.6-luna`, reasoning off (`reasoning_effort="none"`), parallel (`judge_run(workers=N)`).** Switched from `gpt-5.4` on 2026-09-10 after a side-by-side on the 1188 s1_speech v3 answers: luna agrees with gpt-5.4 on 76% of `lean` (Pearson +0.67) and reports the same memory−bare effect size and sign, but runs ~60x faster. It is **not** bit-reproducible (no temperature, no seed); `gpt-5.4` stays reachable with `BENCH_JUDGE_MODEL=gpt-5.4` for a reproducibility check. luna reads ~0.06 further left on levels, so compare **differences**, not absolute means, across judges.
