"""s2_proposal v1 must be a byte-for-byte port of the bench surface.
...
hashes to the historical ``judge_id``.
"""
...
from bench_v2.tasks.s2_proposal.v1 import pilot as v1
from bench_v2.types import Item

BENCH_S2 = "bench/surfaces/tasks/s2_proposal/__init__.py"
...
def bench_surface():
    ...
    registry.load_all()
    return registry.get_surface("s2_proposal")()
...
def test_plan_is_cpu_only(capsys):
    ...
    assert "s2_proposal" in out and "trials" in out
