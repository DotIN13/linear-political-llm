"""s6_describe v1 must be a byte-for-byte port of the bench surface.
...
through both trees and compares the objects, it compares the deterministic
politics reader (both nets), and it asserts the judge spec still hashes to the
historical ``judge_id``.
"""
...
from bench_v2.tasks.s6_describe.v1 import pilot as v1
from bench_v2.types import Item

BENCH_S6 = "bench/surfaces/tasks/s6_describe/__init__.py"
...
def bench_surface():
    ...
    registry.load_all()
    return registry.get_surface("s6_describe")()
...
@pytest.mark.parametrize("text", [
    "She is a thoughtful citizen who follows politics closely.",
    "He loves hiking, cooking, and his golden retriever.",
])
def test_reader_matches_bench(text):
    from bench.types import Response as BenchResponse

    from bench_v2.types import Response as V2Response

    bench_out = bench_surface().extract(BenchResponse(text=text), None)
    v2_out = v1.read(V2Response(text=text), None)
    assert v2_out.to_dict() == bench_out.to_dict()


def test_prompt_matches_bench():
    from bench.surfaces.shared.prompts import text

    assert v1.PROMPT == text(BENCH_S6)


def test_judge_spec_matches_bench():
    from bench.judges.specs import judge_specs

    bench_spec = judge_specs()["s6_describe"]
...
def test_plan_is_cpu_only(capsys):
    ...
    assert "s6_describe" in out and "trials" in out
