"""s5_letter v1 must be a byte-for-byte port of the bench surface.
...
through both trees and compares the objects, it compares the deterministic topic
reader, and it asserts the judge spec still hashes to the historical ``judge_id``.
"""
...
from bench_v2.tasks.s5_letter.v1 import pilot as v1
from bench_v2.types import Item

BENCH_S5 = "bench/surfaces/tasks/s5_letter/__init__.py"
...
def bench_surface():
    ...
    registry.load_all()
    return registry.get_surface("s5_letter")()
...
def test_reader_matches_bench():
    from bench.types import Response as BenchResponse

    from bench_v2.types import Response as V2Response

    text = "Please support climate action and renewable energy; the environment matters."
...
def test_plan_is_cpu_only(capsys):
    ...
    assert "s5_letter" in out and "trials" in out
