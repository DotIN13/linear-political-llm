"""s7_family_chat v2 -- reply to a relative's message, full scheme x persona
factorial.

v1 was the round-6 recipe ported from ``bench`` byte for byte: two schemes
(``chat``/``agentic``), no persona factor, and a judge (the shared
``s2_proposal`` rubric) with no overall ``lean``. So it cannot speak to the
research question (PREFERENCES.md): does the political lean of the user's photos
shift the output, gated by the memory instruction and the delivery mechanism?

The message already flows through ``question_fn`` into every scheme's final turn,
so v2 needs no transcript surgery. It crosses the two gates in:

* the **variant** is the persona instruction (``bare``/``memory``), like s1/s3/s5/s8;
* the **schemes** are the full set -- ``chat`` / ``agentic`` / ``agentic_live`` --
  with the task's own :data:`SCHEME_STYLE`, so the agentic wording is about
  replying to a relative, not s3's news digest;
* the **judge** leads with overall ``lean`` plus ``economic`` and ``social``
  before the framing dimensions, so the dependent variable is a continuous
  left-right score and the proof is the paired ``memory - bare`` per scheme x
  image-lean bucket.

The twelve messages are supplied (the topic is the prompt variant), each states
its issue without taking a side, and the same twelve issues are used by
s8_letter_answered -- so s7 vs s8 is a task-shape contrast on identical topics
(a group-chat reply vs a formal letter).

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s7_family_chat.v2.pilot plan
    python -m bench_v2.tasks.s7_family_chat.v2.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s7_family_chat.v2.pilot judge
    python -m bench_v2.tasks.s7_family_chat.v2.pilot summary
    python -m bench_v2.tasks.s7_family_chat.v2.pilot all

``plan`` and ``summary`` are CPU-only; ``judge`` needs an API key; ``run`` needs
whatever the adaptor needs. Records go to ``runs/bench_v2/s7_family_chat/v2``.
"""
...
from bench_v2.tasks.s7_family_chat.v2.judge_spec import JUDGE
from bench_v2.types import Item, Outcome, Response, Trial

TASK = "s7_family_chat"
VERSION = "v2"
SURFACE = TASK
TITLE = "reply to a relative's message in a family group chat"
HERE = Path(__file__).resolve().parent          # v2's own judge
TASK_DIR = HERE.parent                          # the shared message pool and wrapper

# The task's agentic wording. s3's news-digest wording is the default in the
# scheme builder because s3 is what it was written for; s7 must not inherit it.
...
ROW_META = ("domain", "topic", "lean", "message")

_ROWS, _HEADER = load_pool(TASK_DIR / "messages_v1.jsonl")
BY_ID: dict[str, dict[str, Any]] = {row["mid"]: row for row in _ROWS}
QUESTION_IDS: tuple[str, ...] = tuple(row["mid"] for row in _ROWS)
...
DATASET_HASH = dataset_fingerprint(_ROWS, "mid", "message")
...
def item_bucket(item: Item) -> str:
    ...
    for older rows that predate the covariate. Same rule as s1/s3/s5/s8.
    """
...
def variants() -> list[dict[str, Any]]:
    """Scheme x message x persona variant (bare / memory)."""
    return [{"scheme": scheme, "question": mid, "clause": persona}
            for scheme in SCHEMES for mid in QUESTION_IDS
            for persona in PERSONA_VARIANTS]


def question(mid: str) -> str:
    """The wrapper with one message in it -- the shipped wording, rendered."""
    return render(TASK_DIR / "message.j2", message=BY_ID[mid]["message"])
...
def build(item: Item, condition: str, variant: dict[str, Any] | None = None,
    ...
    mid = str((variant or {}).get("question", ""))
    if mid not in BY_ID:
        raise ValueError(f"{SURFACE}: unknown message {mid!r}; known: {sorted(BY_ID)}")
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--mid", action="append", default=None,
                        help="repeatable; restrict to these message ids")
...
def main(argv: list[str] | None = None) -> int:
    ...
    chosen_variants = [v for v in variants()
                       ...
                       and (not args.mid or str(v.get("question")) in args.mid)]
