...
Ported from ``bench/surfaces/tasks/s7_family_chat`` byte for byte so the two trees
can be compared: the same twelve messages in the same order, the same wrapper, the
same two conversation schemes, and a judge that hashes to the same
``s2_proposal`` rubric. ``bench_v2/tests/test_all_tasks_parity.py`` asserts the
build matches.

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s7_family_chat.v1.pilot plan
    python -m bench_v2.tasks.s7_family_chat.v1.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s7_family_chat.v1.pilot judge
    python -m bench_v2.tasks.s7_family_chat.v1.pilot summary
    python -m bench_v2.tasks.s7_family_chat.v1.pilot all       # run -> judge -> summary

``plan`` and ``summary`` are CPU-only; ``judge`` needs an API key; ``run`` needs
whatever the adaptor needs. Records go to ``runs/bench_v2/s7_family_chat/v1``.
"""
...
def question(qid: str) -> str:
    ...
    return render(TASK_DIR / "message.j2", message=BY_ID[qid]["message"])
