"""The s1_speech judge spec -- the whole thing, owned by the task.

There is no shared spec module: which fields s1 is judged on, what the labels
...
Changing what s1 is judged on is an edit inside ``tasks/s1_speech/`` and cannot
...
# --- the labels s1 returns ----------------------------------------------------
class S1Labels(JudgeLabels):
    lean: Lean | None
    economic: Lean | None
    social: Lean | None
    foreign_policy: Lean | None
    institutional_trust: Lean | None
    formality: Five | None
    optimism: Five | None
...
LABEL_MAP: dict[str, dict[str, float]] = {
    ...
    "foreign_policy": LEAN_MAP, "institutional_trust": LEAN_MAP,
    "formality": FIVE_MAP, "optimism": FIVE_MAP, "concreteness": FIVE_MAP,
}
FIELDS: list[str] = ["lean", "economic", "social", "foreign_policy",
                     "institutional_trust", "formality", "optimism", "concreteness"]
...
JUDGE = make_spec(SURFACE, SYSTEM_PROMPT, S1Labels, LABEL_MAP, FIELDS)
