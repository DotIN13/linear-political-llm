...
The two-topic pick-from-pool surface, ported from ``bench`` byte for byte so the
two trees can be compared: the two asks, the pool, the numbering, the parser and
the dependent variable are the same strings and the same arithmetic.
``bench_v2/tests/test_all_tasks_parity.py`` asserts the built trials are equal.

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s12_explain.v1.pilot plan
    python -m bench_v2.tasks.s12_explain.v1.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s12_explain.v1.pilot summary
    python -m bench_v2.tasks.s12_explain.v1.pilot all       # run -> summary

``plan`` and ``summary`` are CPU-only; ``run`` needs whatever the adaptor needs.
Records go to ``runs/bench_v2/s12_explain/v1``.
"""
...
VERSION = "v1"
SURFACE = TASK
TITLE = "pick three points for a twelve-year-old"
FAMILY = "pick_from_pool"
TASK_DIR = Path(__file__).resolve().parents[1]
TOPICS = ["inflation", "sky_blue"]
ASKS = {t: render(TASK_DIR / f"ask_{t}.j2") for t in TOPICS}
PROMPT = ASKS[TOPICS[0]]
ROWS, HEADER = load_pool(TASK_DIR / "points.jsonl")
CONDITIONS: tuple[str, ...] = ("photos", "no_photos")
SCHEMES: tuple[str, ...] = ("chat", "agentic")
MAX_NEW_TOKENS = 700
N_PICKS = 3
ID_FIELD = "pid"
QUESTION_IDS = list(TOPICS)
