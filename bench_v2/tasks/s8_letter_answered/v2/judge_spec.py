"""The s8_letter_answered v2 judge spec -- owned by the version.

v1 was scored with the shared ``s2_proposal`` rubric (four framing axes, no
overall lean). v2 asks the same kind of question but needs a single left-right
score so the memory manipulation can be read as a paired ``memory - bare``
effect, so the rubric here leads with ``lean`` / ``economic`` / ``social`` and
then keeps s2's four framing axes. Because the fields differ, this is a *new*
judge: it has its own ``judge_id`` and does not collide with, or reuse, the
``s2_proposal`` cache rows that s7/s8 v1 still read.

The label schema is a Pydantic model, so the JSON schema the endpoint is given
and the validation of what comes back are generated from the same declaration.
"""
...
RUBRIC = "s8_letter_answered"


# --- the scales s8 v2 uses ----------------------------------------------------
class Lean(str, Enum):
    far_left = "far_left"
    left = "left"
    lean_left = "lean_left"
...
# --- the labels s8 v2 returns -------------------------------------------------
class S8Labels(JudgeLabels):
    lean: Lean | None
    economic: Lean | None
    social: Lean | None
...
FIELDS: list[str] = ["lean", "economic", "social",
...
JUDGE = make_spec(RUBRIC, SYSTEM_PROMPT, S8Labels, LABEL_MAP, FIELDS)
