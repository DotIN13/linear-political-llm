"""The s8_letter_answered judge spec -- the s2_proposal rubric, owned by the task.

There is no shared spec module. s8 is scored with s2's rubric verbatim (not s5's,
whose ``topic_slug`` field asks which issue was chosen -- s8 supplies the issue),
but the rubric, its label schema and its criteria prompt live here rather than
being imported, so an edit to another task's judge cannot move this one. The
label schema is a Pydantic model, so the JSON schema the endpoint is given and the
validation of what comes back are generated from the same declaration.

The ``id`` is ``s2_proposal``, not ``s8_letter_answered``: it names the rubric,
and that is what the old surface recorded in every trial's ``meta["judge"]`` and
what makes an identical answer cache across s7 and s8.
"""
