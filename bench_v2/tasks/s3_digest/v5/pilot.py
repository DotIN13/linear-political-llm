"""s3_digest v5 -- v4 plus a photo of the user themselves (EasyPortrait).

Same ask, reader, deal, stimuli and scheme set as v4 (``chat`` / ``agentic`` /
``agentic_live`` x ``bare`` / ``memory``; the enhanced ``headlines_v3.jsonl``).
The one change is **additional context**: every trial also carries an EasyPortrait
portrait -- a photo of the user -- delivered the way each scheme delivers memory.

* ``chat``         -- the portrait sits in the first user turn under
  ``"This is a photo of me."``
* ``agentic``      -- ``/memory/me`` is a third memory directory the agent lists
  and opens, alongside ``/memory/hometown`` and ``/memory/preferences``.
* ``agentic_live`` -- the same, inserted after the question.

The portrait is chosen per item from ``portraits_v1.jsonl``, **aligned with the
item's image bucket**: a left-leaning user's photos get a left-leaning portrait,
and so on. Both buckets come from the same ``combined_ideology`` probe (the item
bucket from the fixed +/-0.5 cut, the portrait bucket from rank tertiles, because
the portrait scores do not straddle zero). The portrait is present in *both*
persona variants, so it is context, never a stand-in for the memory instruction.

The comparison of record is v5 against v4: the same run with and without the face.

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s3_digest.v5.pilot plan
    python -m bench_v2.tasks.s3_digest.v5.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s3_digest.v5.pilot summary

``plan`` and ``summary`` are CPU-only; ``run`` needs whatever the adaptor needs.
Records go to ``runs/bench_v2/s3_digest/v5``.
"""
...
from bench_v2.paths import REPO_ROOT, items_dir, runs_dir
...
HEADLINES, HEADLINES_META = load_pool(TASK_DIR / "headlines_v3.jsonl")
PORTRAITS, PORTRAITS_META = load_pool(TASK_DIR / "portraits_v1.jsonl")
...
# --- the portrait (EasyPortrait) ---------------------------------------------
PORTRAITS_BY_BUCKET: Dict[str, List[Dict[str, Any]]] = {
    b: [p for p in PORTRAITS if p.get("bucket") == b] for b in BUCKETS
}


def portrait_path(row: Dict[str, Any]) -> str:
    """Resolve the pool's repo-relative path to an absolute one."""
    raw = str(row["image_path"])
    return raw if Path(raw).is_absolute() else str(Path(REPO_ROOT) / raw)


def portrait_for_item(item: Item, seed: int) -> Dict[str, Any]:
    """A portrait whose lean matches the item's bucket, fixed per (item, seed).

    The choice is a function of the item id and the seed only, so every scheme and
    both persona variants of one item show the *same* face -- the scheme contrast
    stays within-item.
    """
    rng = random.Random(_order_seed(item.item_id, seed))
    pool = PORTRAITS_BY_BUCKET.get(item_bucket(item)) or PORTRAITS
...
def build(item: Item, condition: str, variant: dict[str, Any] | None = None,
    ...
    portrait = portrait_for_item(item, seed)

    def meta_extra(_variant, _order, _qid):
        return {
            "portrait_record_id": portrait["record_id"],
            "portrait_record_name": portrait["record_name"],
            "portrait_image_mean": portrait["image_mean"],
            "portrait_bucket": portrait["bucket"],
        }
    ...
    return build_base(
        ...
        portrait=portrait_path(portrait), portrait_name=PORTRAIT_NAME,
        meta_extra=meta_extra, scheme_style=NEWS_DIGEST_STYLE,
    )
