"""s3_digest v4 -- enhanced headlines, plus a realistic agentic scheme.

Same ask, reader, deal and stimuli as v3 (six topics, both sides; the enhanced
``headlines_v3.jsonl``). The change is the **scheme set**: v4 runs ``chat`` and
both agentic shapes side by side --

* ``agentic``      -- v3's shape: the memory lookup happens *before* the question
  (a user turn describes the directories, the search runs, then the ask).
* ``agentic_live`` -- the question comes **first**, the machine then inserts the
  list_dir/view_image calls and their results in response, and the model's
  generation is the answer after the last tool result -- one turn, the way a
  modern coding agent runs.

So the agentic manipulation can be compared with and without the realistic turn
order in a single run.

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s3_digest.v4.pilot plan
    python -m bench_v2.tasks.s3_digest.v4.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s3_digest.v4.pilot summary

``plan`` and ``summary`` are CPU-only; ``run`` needs whatever the adaptor needs.
Records go to ``runs/bench_v2/s3_digest/v4``.
"""
