"""s3_digest v3 -- pick five of twelve news stories (enhanced headlines).

Same ask, reader and deal as v2: six of the pool's twelve topics, both sides of
each, twelve stories (six left, six right) in random order. The only change is
the stimulus text -- the headlines are the **enhanced** rewrites from
``headlines_v3.jsonl`` (original headline dropped; left = slant -1, right = +1)
instead of the ``headlines_v2.jsonl`` wire headlines. The build is therefore
neither byte-identical to ``bench`` nor to v2.

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s3_digest.v3.pilot plan
    python -m bench_v2.tasks.s3_digest.v3.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s3_digest.v3.pilot summary

``plan`` and ``summary`` are CPU-only; ``run`` needs whatever the adaptor needs.
Records go to ``runs/bench_v2/s3_digest/v3``.
"""
