"""s3_digest v2 -- pick five of twelve news stories (six topics, both sides).

Same ask and reader as v1; the only change is the deal. **v1** showed one side of
each of the pool's twelve topics (twelve topics, six left, six right). **v2**
draws six of the twelve topics and shows **both** sides of each -- the left and
the right headline covering the same story -- so the twelve stories are six
balanced pairs (six left, six right, six topics) in random order. The build is
therefore no longer byte-identical to ``bench``.
