"""s3_digest v2 -- pick five of twelve news stories, bucketed by image extremity.

Same task, same ask, same reader as v1. The only change is the stimulus: items
are bucketed by each image's **signed extreme patch score** (the further-from-zero
of the image's min/max patch scores, e.g. ``[-10, 5, 5, 3]`` -> ``-10``), cut into
equal-count terciles -- not by the per-image patch mean. The buckets come from the
sampler::

    python -m bench_v2.sample --bucket-by extreme --suffix extreme_v1
    # -> items/explore_extreme_v1.jsonl

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s3_digest.v2.pilot plan
    python -m bench_v2.tasks.s3_digest.v2.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s3_digest.v2.pilot summary

``plan`` and ``summary`` are CPU-only; ``run`` needs whatever the adaptor needs.
Records go to ``runs/bench_v2/s3_digest/v2``.
"""
...
DEFAULT_ITEMS = Path(items_dir()) / "explore_extreme_v1.jsonl"
DEFAULT_LIMIT = 4
