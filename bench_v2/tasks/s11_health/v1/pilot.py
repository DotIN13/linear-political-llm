...
The rank-pool surface, ported from ``bench`` byte for byte so the two trees can be
compared: the two asks, the pool, the numbering, the parser and the dependent
...
Run the whole experiment, or stop at a stage::
    ...
    python -m bench_v2.tasks.s11_health.v1.pilot plan
    python -m bench_v2.tasks.s11_health.v1.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s11_health.v1.pilot summary
    python -m bench_v2.tasks.s11_health.v1.pilot all       # run -> summary

``plan`` and ``summary`` are CPU-only; ``run`` needs whatever the adaptor needs.
Records go to ``runs/bench_v2/s11_health/v1``.
"""
...
from bench_v2.helpers.readers import (
    FORMAT_RANK_N, detect_refusal, numbered_question, parse_ranking, rank_weights,
...
VERSION = "v1"
SURFACE = TASK
TITLE = "rank six ways of dealing with a symptom"
FAMILY = "rank_pool"
TASK_DIR = Path(__file__).resolve().parents[1]
SCENARIOS = ["sleep", "allergies"]
ASKS = {s: render(TASK_DIR / f"ask_{s}.j2") for s in SCENARIOS}
PROMPT = ASKS[SCENARIOS[0]]
ROWS, HEADER = load_pool(TASK_DIR / "options.jsonl")
CONDITIONS: tuple[str, ...] = ("photos", "no_photos")
SCHEMES: tuple[str, ...] = ("chat", "agentic")
MAX_NEW_TOKENS = 700
ID_FIELD = "oid"
QUESTION_IDS = list(SCENARIOS)
...
def _ask(qid: str | None) -> str:
    return ASKS[qid or SCENARIOS[0]]
...
def _read_extra(trial: Trial | None) -> dict[str, Any]:
    return {"scenario": _trial_qid(trial)}


def _dv(shown: list[dict[str, Any]], ranking: list[int]) -> dict[str, Any]:
    weights = rank_weights(len(shown))
    score = 0.0
    rank_of_clinician: int | None = None
...
def _read_deterministic(text: str, shown: list[dict[str, Any]],
    ...
    ranking = parse_ranking(text, len(shown))
    if ranking is None:
        return {"parsed": False, "ranking": None, **_read_extra(trial)}
    return {
        "parsed": True,
        "ranking": ranking,
        "ranked_ids": [shown[i - 1][ID_FIELD] for i in ranking],
        **_read_extra(trial),
        **_dv(shown, ranking),
    }
