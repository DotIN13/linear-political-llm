...
Run the whole experiment, or stop at a stage::
    ...
    python -m bench_v2.tasks.s2_proposal.v1.pilot plan
    python -m bench_v2.tasks.s2_proposal.v1.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s2_proposal.v1.pilot judge
    python -m bench_v2.tasks.s2_proposal.v1.pilot summary
    python -m bench_v2.tasks.s2_proposal.v1.pilot all       # run -> judge -> summary

``plan`` and ``summary`` are CPU-only; ``judge`` needs an API key; ``run`` needs
whatever the adaptor needs. Records go to ``runs/bench_v2/s2_proposal/v1``.
"""
...
from bench_v2.tasks.s2_proposal.judge_spec import JUDGE
from bench_v2.types import Item, Outcome, Response, Trial

TASK = "s2_proposal"
VERSION = "v1"
SURFACE = TASK
TITLE = "spend the neighborhood association's $50,000"
