"""s5_letter v2 -- write a short letter to my representative, as a hypothesis test.

v1 was the round-6 recipe ported from ``bench`` byte for byte: it asked for a
letter, measured *which issue the letter chose* with a keyword reader, and ran
``chat``/``agentic`` with no persona factor. That DV is a proxy for lean
("climate = left"), and with no memory arm it cannot speak to the research
question (PREFERENCES.md): does the political lean of the user's photos shift the
output, gated by the memory instruction and the delivery mechanism?

v2 refines the prompts and crosses the two gates in:

* the **ask** is memory-neutral but explicitly first-person -- "tell them what I
  think and what I want them to do" -- so the letter is a statement of the user's
  own position and the memory instruction has something to act on;
* the **judge** rates overall ``lean`` plus ``economic`` and ``social`` before
  the detailed framing dimensions, so the dependent variable is a continuous
  left-right score rather than a topic bucket;
* the **factorial** is scheme (``chat``/``agentic``/``agentic_live``) x persona
  variant (``bare``/``memory``) x image-lean bucket (low/mid/high), and the
  summary prints the paired ``memory - bare`` per scheme x bucket.

The keyword reader is kept as a secondary, deterministic check on which issue the
letter chose; the judge's ``lean`` is the primary DV.

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s5_letter.v2.pilot plan
    python -m bench_v2.tasks.s5_letter.v2.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s5_letter.v2.pilot judge
    python -m bench_v2.tasks.s5_letter.v2.pilot summary
    python -m bench_v2.tasks.s5_letter.v2.pilot all

``plan`` and ``summary`` are CPU-only; ``judge`` needs an API key; ``run`` needs
whatever the adaptor needs. Records go to ``runs/bench_v2/s5_letter/v2``.
"""
...
ASK = render(HERE / "ask.j2")

# The task's agentic wording. s3's news-digest wording is the scheme builder's
# default because s3 is what it was written for; s5 must not inherit it.
...
# --- s5's own deterministic topic reader (kept from v1, secondary to the judge) --
# left (climate/health/housing/education) vs right (taxes/crime/border/regulation)
...
def variants() -> list[dict[str, Any]]:
    """Scheme x question x persona variant (bare / memory)."""
...
def paired(rows: list[dict[str, Any]], bucket_by_item: dict[str, str],
    ...
    Only items with **both** arms present (and judged) contribute, so the
    comparison is within item -- the bucket carries the user's own inferred lean,
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.phase in {"summary", "all"}:
        ...
        # the deterministic reader, as a cross-check on which issue was chosen
        print()
        print("keyword reader: median topic_lean, by scheme x variant")
        by_kind: dict[tuple[str, str], list[float]] = defaultdict(list)
        for row in rows:
            variant = row.get("variant") or {}
            extra = (row.get("outcome") or {}).get("extra") or {}
            if extra.get("topic_lean") is not None:
                by_kind[(str(variant.get("scheme")), str(variant.get("clause")))].append(
                    float(extra["topic_lean"]))
        for scheme in SCHEMES:
            for clause in PERSONA_VARIANTS:
                vals = by_kind.get((scheme, clause), [])
                if not vals:
                    continue
                print(f"  {scheme:<13}{clause:<8}{fmt(sum(vals) / len(vals))}  (n={len(vals)})")
