...
The ranked-pick-from-pool surface, ported from ``bench`` byte for byte so the two
trees can be compared: the ask, the pool, the numbering, the parser and the
weighted dependent variable are the same strings and the same arithmetic.
``bench_v2/tests/test_all_tasks_parity.py`` asserts the built trials are equal.

Run the whole experiment, or stop at a stage::

    python -m bench_v2.tasks.s14_outfits.v1.pilot plan
    python -m bench_v2.tasks.s14_outfits.v1.pilot run --limit 4 --adaptor local_hf
    python -m bench_v2.tasks.s14_outfits.v1.pilot summary
    python -m bench_v2.tasks.s14_outfits.v1.pilot all       # run -> summary

``plan`` and ``summary`` are CPU-only; ``run`` needs whatever the adaptor needs.
Records go to ``runs/bench_v2/s14_outfits/v1``.
"""
...
from bench_v2.helpers.readers import (
    FORMAT_PICK_N, detect_refusal, numbered_question, parse_picks, refusal_match,
    word_count,
)
...
VERSION = "v1"
SURFACE = TASK
TITLE = "shortlist three suits, best first"
FAMILY = "pick_from_pool"
TASK_DIR = Path(__file__).resolve().parents[1]
PROMPT = render(TASK_DIR / "ask.j2")
ROWS, HEADER = load_pool(TASK_DIR / "outfits.jsonl")
...
def _weighted(shown: list[dict[str, Any]], picks: list[int], field: str) -> float:
...
def _dv(shown: list[dict[str, Any]], picks: list[int]) -> dict[str, Any]:
    return {
        ...
        "right_pick_w": _weighted(shown, picks, "right_c"),
        "labour_pick_w": _weighted(shown, picks, "labour_c"),
        "origin_pick_w": _weighted(shown, picks, "origin_c"),
    }
