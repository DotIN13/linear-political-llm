...
def test_the_exchange_is_ask_question_answer():
    ...
    trial = _surface().build(_item(), "C", {"scheme": "chat", "prompt": "c02"})
...
def test_the_opening_ask_is_s5s_verbatim():
    ...
    assert _surface().build(_item(), "C", {"scheme": "chat", "prompt": "c01"}) \
...
def test_both_schemes_get_the_same_exchange():
    s = _surface()
    chat = s.build(_item(), "C", {"scheme": "chat", "prompt": "c05"})
    agentic = s.build(_item(), "C", {"scheme": "agentic", "prompt": "c05"})
...
def test_two_turns_longer_than_the_plain_generation_shape():
    ...
    trial = _surface().build(_item(), "C", {"scheme": "chat", "prompt": "c01"})
...
def test_one_variant_per_concern_per_scheme():
    ...
    for scheme in ("chat", "agentic"):
        assert {v["prompt"] for v in variants if v["scheme"] == scheme} == set(s.cids())


def test_trial_records_which_concern():
    ds = _surface().build(_item(), "C", {"scheme": "chat", "prompt": "c10"}).meta["dataset"]
    assert ds["cid"] == "c10"
...
def test_unknown_concern_is_an_error_not_a_silent_default():
    with pytest.raises(ValueError, match="unknown concern"):
        _surface().build(_item(), "C", {"scheme": "chat", "prompt": "c99"})


def test_no_image_control_keeps_the_exchange_and_drops_the_pixels():
    trial = _surface().build(_item(), "E", {"scheme": "chat", "prompt": "c01"})
...
def test_no_prefill():
    s = _surface()
    assert s.prefill_variants == [] and s.prefill_text is None
