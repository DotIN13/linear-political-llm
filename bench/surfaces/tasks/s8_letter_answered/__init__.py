...
Where its wording lives now, task seven of the eight having set the pattern:

  prompts/assistant_asks.txt     the scripted assistant turn
  prompts/prefill.json           the prefilled opening -- json because it ends in a
                                 blank line and that blank line is the mechanism
  prompts/concerns_v1.jsonl      the twelve concerns
  prompts/concerns_v1.meta.json  version, design, and the framing / wording rules

**There is no ask.txt here, and that is deliberate.** The opening ask is s5's prompt,
imported rather than copied, so s8 is provably s5 plus the clarifying exchange and
nothing else -- ``test_letter_surface`` asserts the two are equal. A copy in this
task's ``prompts/`` would satisfy the layout and quietly permit exactly the drift the
import exists to prevent. It is the one place the tasks/ shape is bent, and it is bent
on purpose.
"""
...
from bench.surfaces.shared.prompts import pool, read_pool, strings, text
from bench.surfaces.shared.surface import GenerationSurface
from bench.surfaces.tasks.s5_letter import PROMPT as OPENING_ASK

DOMAINS = ("domestic", "foreign")

#: Where this task's pool lives. ``load_dataset`` reads it and its sibling header.
DATASET_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "prompts", "concerns_v1.jsonl")
...
ASSISTANT_ASKS = text(__file__, "assistant_asks.txt")


def load_dataset(path: Optional[str] = None) -> Dict[str, Any]:
    """The twelve concerns and their header, validated.
    ...
    was -- header keys plus ``concerns`` -- so callers did not change.
    """
    if path is None:
        rows, header = pool(__file__, "concerns_v1")
        path = DATASET_PATH
    else:
        rows, header = read_pool(path)
    data: Dict[str, Any] = {**header, "concerns": rows}
    if not rows:
...
def dataset_fingerprint(data: Dict[str, Any]) -> str:
    ...
    No longer the *only* guard: the pool moved inside ``bench/surfaces/`` and is hashed
    by content. It stays because it is what a reader of a single trial row can check.
    """
...
class AnsweredLetterSurface(GenerationSurface):
    ...
    prefill_text = strings(__file__, "prefill.json")["prefill"]
    ...
    def build(self, item, condition, variant=None, seed=None):
        ...
        from bench.surfaces.shared.conditions import normalise_condition
        if normalise_condition(condition) == "no_photos":
            # The no-photos condition strips the *persona* framing, not the task's
            # own shape. This surface's task IS the exchange -- the model asks what
            # the issue is and the user answers -- so the opening request has to be
            # restored here, because the bare `question` is only the concern string
            # and means nothing alone ("Gun laws in this state.").
            messages = [{"role": "user",
