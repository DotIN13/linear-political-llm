...
def encode_prompt(processor, prompt):
    """Encode vision model inputs"""
    messages = [
        {
            "role": "user",
            "content": [
...
def extract_and_save_features(model, tokenizer, model_path, df_prompts, output_dir="results", prefix="politician", device=None, mode="text"):
    """
    ...
    if mode == "vision":
        encoded = [encode_prompt(tokenizer, p)['input_ids'].to(device) for p in df_prompts['prompt']]

    labels = df_prompts['label'].values

    # Extract features
    if mode == "text":
        num_hidden_layers = model.config.to_dict().get("num_hidden_layers")
        HEADS = [f"model.layers.{i}.self_attn.head_out" for i in range(num_hidden_layers)]
    if mode == "vision":
        num_hidden_layers = model.config.to_dict().get("text_config", {}).get("num_hidden_layers")
        HEADS = [f"model.language_model.layers.{i}.self_attn.head_out" for i in range(num_hidden_layers)]
    
    head_wise_hidden_states_list = []

    for prompt in tqdm(encoded, total=len(encoded)):
        with torch.no_grad():
            with TraceDict(model, HEADS) as ret:
                output = model(prompt.to(device))
                head_wise_hidden_states = [ret[head].output.squeeze().detach().cpu().to(dtype=torch.float32) for head in HEADS]
