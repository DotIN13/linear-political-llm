...
class HeadwiseLinearProbe(BaseDimensionProbe):
    """Per-head linear probe with Spearman CV scores."""
    ...
    def fit(self, model, prompts: List[Any], labels: Union[List[float], np.ndarray], tokenizer=None, device=None):
        ...
        for li in range(n_layers):
            ...
            for hi in range(n_heads):
                fold_scores = []
                last_model = None
                x_all = get_head_tensor(features, li, hi)
                ...
                ridge_models[li][hi] = last_model
                scores[li, hi] = float(np.mean(fold_scores))
    ...
    def topk_indices(self, k: int = 16) -> np.ndarray:
        if self.scores_ is None:
            raise ValueError("Fit or load the probe before requesting top-k heads.")
        flat = np.argsort(self.scores_.ravel())[::-1][:k]
        h = self.scores_.shape[1]
        return np.stack([flat // h, flat % h], axis=1)
    ...
    def build_steering(
        ...
        top_idx = self.topk_indices(k=k)
        dev = torch.device(device) if isinstance(device, str) else device

        if features_is_ragged(features):
            steering: Dict[int, torch.Tensor] = {}
            for li, layer_features in enumerate(features):
                std = np.std(layer_features[:, 0], axis=0)  # [H, D]
                layer_coef = np.zeros_like(std, dtype=np.float32)
                for l_idx, h_idx in top_idx:
                    if int(l_idx) != li:
                        continue
                    coef = np.asarray(self.weights_[int(l_idx)][int(h_idx)].coef_, dtype=np.float32)
                    layer_coef[int(h_idx)] = coef * std[int(h_idx)]
        ...
        for li, hi in top_idx:
            coef = np.asarray(self.weights_[int(li)][int(hi)].coef_, dtype=np.float32)
            steering[int(li), int(hi)] = coef * std_tensor[int(li), int(hi)]
        return torch.as_tensor(steering, device=dev)

    def score_samples(self, features: Union[np.ndarray, List[np.ndarray]], k: int = 16) -> np.ndarray:
        ...
        for n in range(n_samples):
            vals = []
            for li, hi in top_idx:
                x = get_head_tensor(features, int(li), int(hi))[n].reshape(1, -1)
                vals.append(float(self.weights_[int(li)][int(hi)].predict(x)[0]))
